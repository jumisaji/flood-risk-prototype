...
The tabular rows are generated by `notebooks/eval_metrics.py` into `docs/metrics.json`, so
the table, the `/models` endpoint and the report cannot drift apart. The LSTM row comes from
its own notebook because it needs a 14 day window and drops slightly more early rows: its
fold is 953 days against 954. Comparable in period and size, but not byte identical, which
...
what would let a model beat persistence convincingly. After that, trained models for the
remaining council areas, SMS dispatch through a funded gateway, and a field pilot with a
council or SES unit to validate the alert workflow in practice.
